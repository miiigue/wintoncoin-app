...
function initializeP2PHistoryPage() {
        ...
        elements.historyList.innerHTML = historyOrders.map(order => {
            ...
            return `
                <div class="p2p-order-card">
                    <div><strong>${isBuyer ? 'Comprando' : 'Vendiendo'}</strong> con ${counterparty}</div>
    ...
    function getOrderStatusLabel(status) {
        const map = { released: 'liberada', cancelled: 'cancelada', expired: 'expirada' };
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initializeP2PHistoryPage);
} else {
    initializeP2PHistoryPage();
}

export { initializeP2PHistoryPage };
