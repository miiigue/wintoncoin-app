...
- **Decisión**:
  ...
  - **Abreviación de Estados de Tareas (`contract-interaction.js`)**: Se acortaron los textos de estado de las tarjetas de publicación a un máximo de 2 palabras (ej. "Esperando confirmación", "Puedes comenzar!", "Esperando aprobación", "Pendiente pago"). Esto optimiza el espacio de renderizado vertical en pantallas móviles de baja resolución, evitando que los banners de estado fuercen saltos de línea de 3 niveles y manteniendo una UX compacta y simétrica.
- **Impacto**: Se elimina la inconsistencia visual y de datos entre el panel de administración y el estado real del usuario. Se garantiza la consistencia transaccional y la seguridad del pool de conexiones al evitar condiciones de carrera, y se mantiene la inmutabilidad y la trazabilidad de los datos, reduciendo la latencia de actualización a cero mediante sincronización perezosa (lazy synchronization) al consultar el balance. Adicionalmente, se mejora la visualización móvil de la billetera con tarjetas más compactas y equilibradas.
- **Evidencia**: Modificaciones realizadas en `web3BridgeService.js`, `server.js`, `admin-panel.js` y `contract-interaction.js`.
...
### Filtro de Publicaciones Ocultas
- **Fecha:** 01/06/2026
- **Archivos Modificados:** frontend/contract_interaction.html, frontend/src/pages/contract-interaction.js, backend/src/controllers/publicationController.js
- **Descripci�n:** Se a�adi� un nuevo 'chip' en el Frontend para filtrar las publicaciones ocultadas por el usuario. El Frontend ahora cambia din�micamente entre solicitar publicaciones 'activas' u 'ocultas' dependiendo del filtro seleccionado, enviando la petici�n respectiva al backend. En el backend se implement� el nuevo endpoint /publications/hidden que devuelve de forma segura y auditable exclusivamente aquellas publicaciones excluidas voluntariamente por el usuario (las que ocult� usando la 'X'). Esta mejora respeta el modelo econ�mico y de auditor�a de la plataforma sin alterar en absoluto los estilos (CSS) originales.
