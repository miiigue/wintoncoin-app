...
### 2026-06-15 — Reconciliación Contable de Comisiones Acumuladas y Doble Entrada en Pagos de Impulsores

- **Contexto**: El proceso de distribución de pagos de impulsores (`executeBoosterPayments`) presentaba un desfase de presupuesto y una violación de los principios contables de partida doble. Por un lado, buscaba el presupuesto de pago filtrando estrictamente por comisiones recibidas en el mes de ejecución en `platform_commission_log` (lo que arrojaba `0.0000 BLUE` en meses sin transacciones como junio de 2026, ignorando las comisiones acumuladas previas mostradas en el Dashboard). Por otro lado, no restaba los fondos repartidos a los impulsores de la billetera central de la plataforma (`platform_wallet.total_blue_commission_balance`) ni generaba registros de egreso en el libro mayor, permitiendo crear pasivos en custodia sin contrapartida de caja real.
- **Decisión de Ingeniería**:
  1. **Presupuesto Consolidado de la Plataforma**: Se modificó `executeBoosterPayments` en [boosterService.js](file:///c:/Users/migue/OneDrive/Escritorio/WINTONCOIN/smart-contract/backend/src/services/boosterService.js) para obtener el saldo de comisiones acumuladas directamente de `platform_wallet` usando un bloqueo pesimista `FOR UPDATE` para garantizar consistencia transaccional concurrente.
  2. **Asiento Contable de Egreso y Partida Doble**: Dentro del ciclo de liquidación de cada impulsor, se agregó un `UPDATE` a `platform_wallet` para deducir el monto pagado (`amountToPay`), y se insertó un registro de egreso negativo en `platform_wallet_log` con tipo `booster_payout`.
  3. **Migración de Base de Datos (`061_create_platform_wallet_log.js`)**: Se introdujo una migración transaccional e idempotente para crear la tabla de auditoría `platform_wallet_log` con índices B-Tree de rendimiento. Esta tabla también se integró en la inicialización automática de la base de datos en `databaseInit.js` para nuevos despliegues.
  4. **Pruebas de Integración y Reglas de Inmutabilidad**: Se implementó una suite de pruebas de integración completa en `boosterPaymentsReconciliation.test.js` que simula el ciclo de pagos aislando temporalmente a otros boosters, validando que el saldo de la plataforma disminuya coherentemente y que la bitácora contable registre los egresos sin violar la inmutabilidad física del Ledger General de la base de datos (evitando DELETEs en tablas protegidas).
- **Impacto**: Se restableció la consistencia contable al 100%. Las comisiones acumuladas del Dashboard administrativo ahora reflejan el descuento exacto tras cada distribución de pagos. Se cumple con los estándares bancarios de partida doble (GAAP) y las normativas FinTech de Silicon Valley (prevención de emisión no respaldada/money printing y auditabilidad reproducible del Ledger).
- **Evidencia**:
  ...
  - Migración: [061_create_platform_wallet_log.js](file:///c:/Users/migue/OneDrive/Escritorio/WINTONCOIN/smart-contract/backend/migrations/061_create_platform_wallet_log.js).
  - Pruebas: [boosterPaymentsReconciliation.test.js](file:///c:/Users/migue/OneDrive/Escritorio/WINTONCOIN/smart-contract/backend/__tests__/boosterPaymentsReconciliation.test.js).
