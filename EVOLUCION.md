...
### 2026-07-17 — Manejo de Navegadores In-App (WhatsApp) y Prevención de Pestañas Duplicadas

- **Autor**: Antigravity (AI Engineering)
- **Tipo**: UX/UI Premium y Estabilidad de Sesión (Frontend)
- **Rama**: `fix/email-asterisks-cause-update`
- **Contexto**:
  1. Los usuarios que hacían clic en el enlace de referidos en WhatsApp abrían la app dentro del navegador interno de la red social (WebView sandbox), el cual no comparte cookies ni almacenamiento local con el navegador del sistema, resultando en sesiones vacías o bucles de verificación redundantes.
  2. Si el usuario hacía clic múltiples veces en WhatsApp para abrir la app, el navegador abría una pestaña adicional nueva cada vez, congestionando el navegador móvil del usuario con instancias duplicadas del Dashboard.
- **Solución Implementada**:
  - **Detección de Navegadores In-App (`in-app-detector.js`)**: Creamos un módulo detector de WebViews integrados de redes sociales (WhatsApp, Instagram, Facebook). Al detectar que la app corre allí, despliega un banner de fondo premium y advertencia que orienta visualmente al usuario a pulsar el menú superior/inferior de su OS para "Abrir en el Navegador" (Safari en iOS / Chrome en Android).
  - **Evitación de Pestañas Duplicadas (`tab-manager.js`)**: Implementamos un gestor de pestañas activas usando la API estándar `BroadcastChannel`. Si el Dashboard detecta que ya existe otra pestaña activa corriendo WintonCoin en el mismo navegador, muestra un aviso elegante de tipo Glassmorphism sugiriendo usar la pestaña original para evitar conflictos de saldo/operaciones.
  - **Enrutamiento de Instancia Única PWA (`manifest.json` / `manifest.demo.json`)**: Agregamos la propiedad estándar de la industria `"launch_handler": { "client_mode": "navigate-existing" }` a los archivos de manifiesto. Esto indica al sistema operativo que si el enlace es invocado teniendo la PWA instalada, en lugar de abrir una ventana nueva, reutilice y enfoque la ventana de la PWA ya activa.
- **Impacto**:
  - **Experiencia UX Impecable**: Flujos limpios alineados a los estándares de grandes aplicaciones (como Airbnb o Uber), previniendo saturación de pestañas en los teléfonos móviles de los usuarios y educándolos para salir del sandbox de WhatsApp.
- **Evidencia**: Archivos modificados: `frontend/public/manifest.json`, `frontend/public/manifest.demo.json`, `frontend/src/modules/in-app-detector.js`, `frontend/src/modules/tab-manager.js`, `frontend/src/pages/login.js`, `frontend/src/pages/register.js`, `frontend/src/pages/contract-interaction.js`, `EVOLUCION.md`.
