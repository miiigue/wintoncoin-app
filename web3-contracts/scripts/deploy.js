...
async function main() {
    ...
    const blueToken = await BlueToken.deploy({ gasLimit: 5000000 });
    ...
    const redToken = await RedToken.deploy({ gasLimit: 5000000 });
    ...
    const protocol = await Protocol.deploy(deployer.address, { gasLimit: 5000000 });
    ...
    const treasury = await Treasury.deploy(blueAddress, { gasLimit: 5000000 });
    ...
    const txBlue = await blueToken.setWintonProtocol(protocolAddress, { gasLimit: 1000000 });
    await txBlue.wait();
    ...
    const txRed = await redToken.setWintonProtocol(protocolAddress, { gasLimit: 1000000 });
    await txRed.wait();
    ...
    const txContracts = await protocol.setContracts(blueAddress, redAddress, treasuryAddress, { gasLimit: 1000000 });
    ...
    const txExempt = await blueToken.setExemptFromAmortization(treasuryAddress, true, { gasLimit: 1000000 });
    await txExempt.wait();
